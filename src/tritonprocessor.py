...
class T5TritonProcessor(TritonProcessorBase):
    def __init__(self, hf_model_path="t5-base", host="localhost", port=8000):
        super().__init__(self, hf_model_path, host, port)
...
class BloomTritonProcessor(TritonProcessorBase):
    def __init__(self, hf_model_path="bigscience/bloom-560m", host="localhost", port=8000):
        super().__init__(self, hf_model_path, host, port)
    ...
    def _preprocess(self, string_input):
        ...
        inputs = [
            ...
            httpclient.InferInput(
                "sequence_length", # TODO THIS IS WRONG
                mem_seq_len.shape,
                np_to_triton_dtype(mem_seq_len.dtype),
            ),
            httpclient.InferInput(
                "max_output_len", # TODO THIS IS WRONG
                max_output_len.shape , 
                np_to_triton_dtype(mem_seq_len.dtype)
            ),
            httpclient.InferInput(
